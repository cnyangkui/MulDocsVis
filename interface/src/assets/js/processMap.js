 ...
 * @returns {Array} 数组中每个元素是一个坐标 [x, y]
 */
export function genRandomPoints(projdata, mapExtent, width, numPerCell) {
  let randomPoints = [];
  let innerXNum =
    Math.ceil((mapExtent[2] - mapExtent[0]) / width);
  let innerYNum =
    Math.ceil((mapExtent[3] - mapExtent[1]) / width);
  let xScale = d3
    .scaleQuantize()
    .domain([mapExtent[0], mapExtent[2]])
    .range(d3.range(0, innerXNum));
  let yScale = d3
    .scaleQuantize()
    .domain([mapExtent[1], mapExtent[3]])
    .range(d3.range(0, innerYNum));
  let grid = [];
  for (let i = 0; i < innerXNum; i++) {
    let row = new Array(innerYNum);
    row.fill(0);
    grid.push(row);
  }
  ...
  for (let i = 0; i < innerXNum; i++) {
    for (let j = 0; j < innerYNum; j++) {
      if (grid[i][j] < 10) {
        let diff = numPerCell - grid[i][j];
        while (diff > 0) {
          let x =
            mapExtent[0] + _.random(i * width, (i + 1) * width, true);
          let y =
            mapExtent[1] + _.random(j * width, (j + 1) * width, true);
